...
prepSurvData <- function(surv.formula, data, id, time.long) {
  ...
  if (ctest > 3) {
    stop(
      "More than 3 event types detected; joineR can only model 2 competing risks"
    )
  } else if (ctest == 3) {
    compRisk <- TRUE
  ...
  if (!compRisk) {
    ...
    p2 <- dim(survdat)[2] - 3
    if (p2 > 0) {
      survdat[, 4:dim(survdat)[2]] <- scale(
        survdat[, 4:dim(survdat)[2]],
        scale = FALSE
      )
    }
    survdat2 <- data.frame(
    ...
    if (!all(unique(event) %in% 0:2)) {
      stop(
        "Competing risks data must be coded as 0 (censored), 1 and 2 (causes)"
      )
    }
    ...
    surv.terms <- terms(surv.formula1, data = cbind(survdat, data$baseline))
    attr(surv.terms, "intercept") <- 1
    surv.frame <- model.frame(
      surv.formula1,
      data = cbind(survdat, data$baseline)
    )
    surv.cov <- model.matrix(surv.terms, data = cbind(survdat, data$baseline))
    ...
    p2 <- dim(survdat)[2] - 4
    if (p2 > 0) {
      survdat[, 5:dim(survdat)[2]] <- scale(
        survdat[, 5:dim(survdat)[2]],
        scale = FALSE
      )
    }
