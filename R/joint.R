...
joint <- function(
  ...
  # Sort the data
  sort <- sortJointData(longdat, survdat)
  longdat <- sort$long.s
  survdat <- sort$surv.s
  ...
  ldaests <- longst(longdat, long.formula, model, long.data)
  ...
  if (!compRisk) {
    ...
    fixed <- list(longitudinal = b1, survival1 = b2.a, survival2 = b2.b)
