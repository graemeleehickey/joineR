...
jointdata <- function(
  ...
  # Checks
  if (
    !(is.matrix(longitudinal) | is.data.frame(longitudinal)) &
      notNA(longitudinal)
  ) {
  ...
  if (
    !(is.matrix(baseline) | is.data.frame(baseline) | is.character(baseline)) &
      notNA(baseline)
  ) {
    stop(
  ...
  nm <- names(which(
    !is.na(list(longitudinal = longitudinal, survival = survival))
  ))
  if (length(nm) == 0) {
  ...
  if (length(nm) == 2) {
    ...
    if (sum(sort(patid) != sort(ss)) > 0) {
      stop(
        "Number of subjects different in the longitudinal and survival data frames"
      )
    }
    ...
    if (!(any(is.na(baseline)))) {
      ...
      if (sum(sort(patid) != sort(bb)) > 0) {
        stop(
          "Number of subjects different in the longitudinal and covariates data frame"
        )
      }
      if (length(bb) != dim(baseline)[1]) {
        stop("Same patient with different covariate data")
