...
summary.jointdata <- function(object, ...) {
  ...
  if (any(is.na(object$longitudinal)) & !is.data.frame(object$longitudinal)) {
    ...
    names(out[[2]]) <- c("class")
    row.names(out[[2]]) <- names(object$longitudinal)[
      2:(dim(object$longitudinal)[2])
    ]
  ...
  if (any(is.na(object$survival)) & !is.data.frame(object$survival)) {
    ...
    if (length(unique(object$survival[[nn]])) %in% c(1L, 2L)) {
      ...
      out[[3]] <- data.frame(c(
        sum(object$survival[[nn]] == 1),
        sum(object$survival[[nn]] == 2),
        sum(object$survival[[nn]] == 0)
      ))
      row.names(out[[3]]) <- c(
        "Number of subjects that fail due to cause 1:",
  ...
  if (any(is.na(object$baseline)) & !is.data.frame(object$baseline)) {
    ...
    out[[4]] <- as.data.frame(matrix(
      0,
      ncol = 1,
      nrow = dim(object$baseline)[2] - 1
    ))
    names(out[[4]]) <- c("class")
  ...
  if (is.na(object$time)) {
    ...
    if (length(tt) > 20) {
      out[[
        5
      ]] <- "Unbalanced longitudinal study or more than twenty observation times"
    } else {
      out[[5]] <- tt
    }
  }
