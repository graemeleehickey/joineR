...
#' individuals.
#' 
#' @param indv vector of individual identification, as in the longitudinal
#' data, repeated for each time point.
...
#' longitudinal data, or residuals after fitting a model for the mean response.
#' 
#' @details The empirical variogram in this function is calculated from observed
#' half-squared-differences between pairs of measurements, \eqn{v_ijk = 0.5 * (r_ij-r_ik)^2} and the corresponding time differences \eqn{u_ijk=t_ij-t_ik}.
#' The variogram is plotted for averages of each time lag for the \eqn{v_ijk} for
#' all \eqn{i}.
#' 
...
#' @keywords variogram
#' 
#' @return An object of class \code{vargm} and \code{list} with two elements. The first \code{svar}
#' is a matrix with columns for all values \eqn{(u_ijk,v_ijk)}, and the second
#' \code{sigma2} is the total variability in the data.
#' @export
#' 
#' @examples
#' 
